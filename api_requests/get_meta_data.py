...
def list_cities(user_country: str = None) -> List[Tuple[str, int]]:
    ...
    if data["suggestions"][0]["group"] == "CITY_GROUP":
        lst_city = tuple(
            (i_city["name"], i_city["destinationId"])
...
def list_cities_area(user_city: str = None) -> Tuple[Tuple[str, int]]:
    ...
    lst_city_area = tuple(
        (j["name"], j["geoId"]) for i in data["suggestions"] for j in i["entities"]
    )
...
def list_hotels(
    ...
    response = request(
        "POST", URL_HOTELS, json=payload, headers=config.HAEDERS_RAPID, timeout=1000
    )
    data = json.loads(response.text)
...
def detail_hotel(id_hotel: int):
    ...
    payload = {"currency": "USD", "eapid": 1, "locale": "en_US", "propertyId": id_hotel}
    ...
    try:
        rating_hotel = data["data"]["propertyInfo"]["summary"]["overview"][
            "propertyRating"
        ]["rating"]
    except TypeError:
    ...
    photo_location = data["data"]["propertyInfo"]["summary"]["location"]["staticImage"][
        "url"
    ]
