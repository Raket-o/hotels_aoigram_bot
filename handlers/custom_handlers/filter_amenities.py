""" Модуль фильтра особенности отелей"""
import loader
import logging
import sqlite3
...
logger = logging.getLogger("logger_handler_filter_amenities")

AMENITIES = (
    ("Бесплатный авиатранспорт", "FREE_AIRPORT_TRANSPORTATION"),
    ("Вид на океан", "OCEAN_VIEW"),
    ("Казино", "CASINO"),
    ("Балкон или терраса", "BALCONY_OR_TERRACE"),
    ("Аренда электрокаров", "ELECTRIC_CAR"),
    ("Тренажёрный зал", "GYM"),
    ("Бассейн", "POOL"),
    ("Аквапарк", "WATER_PARK"),
...
@dp.message_handler(state=UserInfoState.set_amenities)
async def get_filter_amenities(message: types.Message, state: FSMContext) -> None:
    """
    Функция get_filter_amenities. Проверят правильно ли выбрана категория фильтра.
    ...
    :return: None
    """
    example_answer = [i[0] for i in AMENITIES]
    if message.text in example_answer:
        method_sort, *_ = [i[1] for i in AMENITIES if i[0] == message.text]
            ...
            try:
                async with state.proxy() as data:
                    ...
                    list_hotels = get_meta_data.list_hotels(region_id=data["id_city_area"],
                                                            ...
                                                            adding_to_the_dict={"amenities": method_sort})
...
Рейгтин: {rating_hotel}"""
                ...
                logger.error("filter_set_amenities")
